...
# 四、加密
Java加密架构是一个在Java平台中为了访问和开发加密功能的框架。它包括用于各种加密服务的API。这些加密服务包括：
- 信息摘要算法
- 数字签名算法
- 对称块加密
- 对称流加密
- 非对称加密
- 基于密码的加密(PBE，Password-based encryption)
- 椭圆曲线密码体制（ECC）
- 密钥协商算法
- 密钥生成器
- 消息认证码（MACs）
...
出于历史的出口管制原因，加密API被组织为两个不同的包。`java.security`包中的类（例如`Signature`和`MessageDigest`）不受出口管制控制。而`javax.crypto`包中的类（例如`Cipher`和`KeyAgreement`）受出口管制控制。

加密接口是基于Provider的，允许多个和互操作的加密实现。有些Provider在软件级别实现加密操作，而有些在硬件令牌中(例如，在智能卡设备上或在硬件加密加速器上)实现那些操作。Provider实现受出口管制的服务必须进行数字签名。

Java平台中包含很多内置的Provider，它们实现了很多常用的加密算法，例如RSA，DSA和ECDSA签名算法；DES，AES和ARCFOUR加密算法；MD5和SHA-1信息摘要算法；以及Diffie-Hellman密钥协商算法。那些默认的Provider在Java代码中实现加密算法。

Java平台还包括一个内置的提供者，它充当一个本地PKCS#11(v2.x)令牌的桥接器。这个叫做SunPKCS11的Provider允许Java应用无缝地访问位于可兼容PKCS11的令牌上的密码服务
...
# 六、认证
认证是确定用户身份的过程。在Java运行时环境中，它是识别执行Java程序的用户的过程。在某些情况下，这个过程可能依赖于“加密”一章中描述的服务(第4章)。

Java平台提供了允许应用程序通过可插入的登录模块执行用户身份验证的API。应用程序调用`javax.security.auth.login`包中的`LoginContext`类，该类依次引用一个配置。该配置指定哪个实现`javax.security.auth.spi.LoginModule`接口的登录模块去执行实际的认证。

由于应用程序只与标准的`LoginContext`API进行通信，所以它们可以独立于底层的插件模块。新的或更新的模块可以插入到应用程序中，而无需修改应用程序本身。图3说明了应用程序和底层登录模块之间的独立性。

![]()

需要注意的是，尽管登录模块是配置在Java平台中的可插入的组件，但它们并不是通过安全Provider插入的。因此，它们不遵循第三章中描述的Provider搜索模型。相反，登录模块是由它们自己的独特配置管理的，如上图所示。

Java平台提供了以下内置的登录模块，它们都在`com.sun.security.auth.module`包中：
- `Krb5LoginModule`为了使用Kerberos协议认证。
- `JndiLoginModule`为了使用LDAP或者NIS数据库进行用户名密码认证。
- `JndiLoginModule`为了用于登录任何类型的密钥存储库，包括PKCS#11环密钥库。

在在两个对等点之间建立安全通信通道的过程中也可以实现身份验证。Java平台提供了许多标准通信协议的实现，下面一节将讨论这些协议。

# 七、安全通信
通过网络传输的数据可以由非预期接收方访问。当数据包括密码和信用卡号码等隐私信息时，必须采取措施保证未经授权的当事人难以理解该数据。确保数据发送到合适的接收方，并且保证在传输过程中数据不会被有意或者无意的修改，也是同样重要的。
